...
// НОВОЕ: Стоимость повышения статуса
$upgrade_cost = $user['user_type'] === 'responsible' ? 0 : 8000; // 8000 рублей с НДС 22%
$can_upgrade  = $user['user_type'] !== 'responsible' && $user['balance'] >= $upgrade_cost;
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        #upgrade-msg { min-height: 20px; font-size: 13px; font-weight: bold; margin-bottom: 8px; }

        /* Статус компонент с кнопкой */
...
<main class="main">
    ...
    <div class="grid-2" style="margin-bottom:24px;">
        ...
        <div class="card" style="display:flex;flex-direction:column;gap:16px;">
            <!-- НОВОЕ: Статус с кнопкой повышения -->
...
<!-- НОВОЕ: Модалка повышения статуса -->
<div class="modal-overlay" id="modal-upgrade" onclick="if(event.target===this)this.classList.remove('open')">
    <div class="modal-box">
        <h3>⭐ Повысить статус</h3>
        <p style="color:#666;font-size:14px;margin:12px 0;">
            Повысьте свой статус на <b>✅ Ответственного</b> за <b>8000 ₽</b> (НДС 22%)
        </p>
        <p style="color:#666;font-size:12px;margin:0 0 16px;">
            Преимущества: бесплатные ставки на многих аукционах
        </p>
        <div style="margin: 10px 0 18px; position: relative;">
            <label style="display:flex; align-items:center; gap:8px; cursor:pointer; color:#0f172a;">
                <input type="checkbox" id="upgrade-express" style="accent-color:#3b82f6;">
                Экспресс-подтверждение за 24 часа (+7000 ₽)
            </label>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center;margin:16px 0;">
            <button class="btn btn-primary" style="flex:1;" onclick="upgradeQR()">
                📱 QR / СБП
            </button>
            <button class="btn btn-primary" style="flex:1;" onclick="upgradeReceipt()">
                🧾 Квитанция
            </button>
        </div>
        <div id="upgrade-msg" style="margin-top:10px;"></div>
        <button class="modal-close" onclick="document.getElementById('modal-upgrade').classList.remove('open')">
            Отмена
        </button>
    </div>
</div>
...
const UPGRADE_COST = 8000; // фиксированная стоимость с НДС 22%
...
function setUpgradeMsg(text, color) {
    const m = document.getElementById('upgrade-msg');
    m.textContent = text; m.style.color = color || '#ef4444';
}

// НОВОЕ: Функции для повышения статуса
...
function upgradeReceipt() {
    ...
    performUpgrade('receipt');
}

function performUpgrade(method) {
    const express = document.getElementById('upgrade-express')?.checked ? 7000 : 0;
    const total = UPGRADE_COST + express;
    setUpgradeMsg('Формируем платёж...', '#0088cc');
    showUpgradeModal(method, total, express);
    setUpgradeMsg('✅ Ссылка на оплату сформирована', '#22c55e');
}

function showUpgradeModal(method, amount = UPGRADE_COST, express = 0) {
    const purpose = `Повышение статуса ID${USER_ID} ${USERNAME}`;
    const finalAmount = amount;

    if (method === 'qr') {
        const qrData = `ST00012|Name=ООО Форсаж|PersonalAcc=40702810101500033019|BankName=ООО Банк Точка|BIC=044525104|CorrespAcc=30101810745374525104|PayeeINN=7728282160|KPP=773001001|Sum=${finalAmount}00|Purpose=${purpose}`;
        window.location.href = `upgrade_qr.php?amount=${finalAmount}&purpose=${encodeURIComponent(purpose)}&id=${USER_ID}`;
    } else {
        window.location.href = `upgrade_receipt.php?amount=${finalAmount}&purpose=${encodeURIComponent(purpose)}&id=${USER_ID}`;
    }
}
