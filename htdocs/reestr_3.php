...
try {
    // Завершаем активные лоты с истекшим временем
    $pdo->exec("
        UPDATE lots 
    ...
    // Для скандинавских лотов, завершённых, но с менее чем 2 ставками → failed
    $pdo->exec("
        UPDATE lots l
    ...
    <?php else: ?>
        <table class="registry-table">
            ...
            <?php foreach ($lots as $lot):
                ...
                // 1) статус в БД finished/failed/cancelled/single, ИЛИ
                // 2) end_time истекло, ИЛИ
                // 3) max_end_time истекло
                $is_finished = in_array($lot['auction_status'] ?? '', ['finished', 'single', 'failed', 'cancelled'])
                               || ($end_ts && $end_ts <= $now_ts)
                               || ($max_end_ts && $max_end_ts <= $now_ts);
                
                // Лот начат, если started_at <= now (или если нет started_at, но end_time ещё не истекло – не начат)
                $is_started = ($started_ts && $started_ts <= $now_ts);
                
                // Вычисляем оставшееся время для активных лотов
                if (!$is_finished) {
                    $diff = PHP_INT_MAX;
                ...
                // ---------- ОПРЕДЕЛЕНИЕ ОТОБРАЖАЕМОГО СТАТУСА ----------
