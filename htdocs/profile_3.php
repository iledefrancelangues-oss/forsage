...
// Действующие заявки (активные лоты, где пользователь подал заявку)
...
// Черновики (незавершённые заявки)
...
// Поданные заявки (на обработке, одобренные, отклонённые)
...
<aside class="sidebar">
    ...
    <nav>
        ...
        <!-- ДОБАВЛЕНА ССЫЛКА НА КОМИССИОННУЮ ПРОДАЖУ -->
        <a href="commission_lots.php" class="nav-item">
...
<main class="main">
    ...
    <!-- Основной контент кабинета (баланс, статус и т.д.) -->
    <div id="main-content">
...
<?php endif; ?>
        ...
        <div class="card" style="margin-top:24px;">
            ...
            <div class="tab-pane active" id="tab-active">
                ...
                <div class="table-wrap">
                     <table>
                        <thead>
                             <tr><th>Лот</th><th>Дата подачи</th><th>Окончание</th><th>Действия</th></tr>
                        </thead>
                        <tbody>
                             ...
                             <tr>
                                 ...
                                 <td>
                                    <span class="status-badge status-<?= $app['application_status'] ?>">
                                        <?= $status_icon[$app['application_status']] ?? '' ?>
                                        <?php
                                        $status_text = ['pending' => 'На рассмотрении', 'approved' => 'Одобрена', 'rejected' => 'Отклонена'];
                                        echo $status_text[$app['application_status']] ?? ucfirst($app['application_status']);
                                        ?>
                                    </span>
                                 </td>
    ...
    <div id="telegram-content" style="display:none;">
        <div class="card">
            ...
            <?php else: ?>
                <p style="color:var(--dim); margin-bottom:16px;">
                    Привяжите Telegram для получения уведомлений о торгах.
                </p>
...
<!-- Модалки и скрипты (оставляем как в оригинале) -->
...
// Переключение между основным контентом, паролем и телеграмом
const mainContent = document.getElementById('main-content');
const passwordContent = document.getElementById('password-content');
const telegramContent = document.getElementById('telegram-content');

document.getElementById('password-tab-link').addEventListener('click', function(e) {
    e.preventDefault();
...
document.getElementById('telegram-tab-link').addEventListener('click', function(e) {
    e.preventDefault();
    document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
    this.classList.add('active');
    mainContent.style.display = 'none';
...
// Переключение вкладок в блоке "Торги"
