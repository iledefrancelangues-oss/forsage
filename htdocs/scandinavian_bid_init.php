...
// ── Проверка бана ─────────────────────────────────────
$ban = checkUserBan($pdo, $user_id); // предполагаем, что она уже есть
...
try {
    ...
    // если используешь auction_status / auction_status_current
    ...
    $actual_cost = getBidCost($user_type, $payment_method); // из bid_config.php
    ...
    if ($payment_method === 'balance') {
        ...
        // Здесь можно создать pending-запись в pending_bids / commission_transactions
        // Сейчас просто считаем, что ставка идёт сразу, а оплата отрабатывается отдельно
    }
    ...
    $base_cash_cost = BID_PRICES[$user_type]['cash']; // как у тебя было
    $new_bid_count  = (int)$lot['real_bid_count'] + 1;

    // Цена лота растёт на шаг + тариф (как и раньше)
    ...
    echo json_encode([
        'success'            => true,
        'pending'            => ($payment_method === 'cash'), // при желании
        'msg'                => 'Ставка принята!',
        'new_price'          => $new_price,
        ...
        'total_bids_revenue' => $total_bids_revenue, // ← это S
    ], JSON_UNESCAPED_UNICODE);
