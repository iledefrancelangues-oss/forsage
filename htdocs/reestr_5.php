    ...
    <?php else: ?>
        <table class="registry-table">
            ...
            <?php foreach ($lots as $lot):
                ...
                // ---- КНОПКА: для неавторизованных редирект на login.php с возвратом ----
                if (empty($_SESSION['user_id'])) {
                    $target_url = $is_scand ? "lot_scandinavian.php?id=$id" : "lot_details.php?id=$id";
                    $login_url = "login.php?redirect=" . urlencode($target_url);
                    $btn_url = $login_url;
                } else {
