...
// ---------- ПОЛУЧАЕМ ТИП ПОЛЬЗОВАТЕЛЯ ДЛЯ ПРОВЕРКИ ПРАВ ----------
$user_type = null;
$can_create_lot = false;

if (!empty($_SESSION['user_id'])) {
    $stmt_user = $pdo->prepare("SELECT user_type FROM users WHERE id = ?");
    $stmt_user->execute([$_SESSION['user_id']]);
    $user_data = $stmt_user->fetch(PDO::FETCH_ASSOC);
    if ($user_data) {
        $user_type = $user_data['user_type'];
        // Только organizer и admin могут создавать лоты
        $can_create_lot = in_array($user_type, ['organizer', 'admin']);
    }
}
// ---------- КОНЕЦ ПРОВЕРКИ ----------
...
<main style="flex:1;">
    ...
    <div class="registry-page">
        <div class="registry-header">
            ...
            <?php if ($can_create_lot): ?>
                <a href="add_lot.php" class="registry-new-lot">
                    ➕ Создать лот
                </a>
            <?php endif; ?>
        </div>
