...
if ($action === 'login') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT id, username, email, password, ban_type, balance, full_name FROM users WHERE username = ? OR email = ?");
...
if ($action === 'register') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO users (username, email, password, full_name, documents, balance, created_at) 
            VALUES (?, ?, ?, ?, ?, 0, NOW())
        ");
        $stmt->execute([
            $username, 
...
if ($action === 'telegram') {
    ...
    try {
        ...
        if ($user) {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO users (username, full_name, telegram_id, balance, created_at) 
                VALUES (?, ?, ?, 0, NOW())
            ");
