<?php
// commission_sale.php - Раздел комиссионной продажи (публичный доступ)
ob_start();
...
<body>
    ...
    function makeOffer(lotId, minPrice) {
        <?php if (!$is_logged_in): ?>
        if (confirm('Для предложения цены необходимо войти в систему. Перейти к авторизации?')) {
    ...
    function contactSeller(lotId) {
        <?php if (!$is_logged_in): ?>
        if (confirm('Для связи с продавцом необходимо войти в систему. Перейти к авторизации?')) {
            window.location.href = 'index.php?action=login';
        }
        return;
        <?php else: ?>
    ...
    function buyReport(lotId) {
        <?php if (!$is_logged_in): ?>
        if (confirm('Для покупки отчёта необходимо войти в систему. Перейти к авторизации?')) {
            window.location.href = 'index.php?action=login';
        }
        return;
        <?php else: ?>
