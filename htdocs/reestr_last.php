    ...
    <?php else: ?>
        <table class="registry-table">
            ...
            <?php foreach ($lots as $lot):
                ...
                // ----- КНОПКА ДЛЯ НЕАВТОРИЗОВАННЫХ (ВЫЗЫВАЕТ МОДАЛЬНОЕ ОКНО) -----
                $lot_url = $is_scand ? "lot_scandinavian.php?id=$id" : "lot_details.php?id=$id";
...
<?php
// Если функция openAuthModal не определена, попробуем найти модальное окно и открыть его вручную
if (empty($_SESSION['user_id'])) {
    echo '<script>
        if (typeof openAuthModal !== "function") {
            window.openAuthModal = function() {
                ...
                if (modal) {
                    ...
                    // Если модального окна нет, покажем стандартное сообщение (не ломает вёрстку)
                    alert("Для участия в торгах необходимо авторизоваться. Пожалуйста, перейдите на главную страницу и войдите.");
                }
            };
        }
    </script>';
