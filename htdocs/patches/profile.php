...
$upgrade_cost = $user['user_type'] === 'responsible' ? 0 : 4990;
        ...
        <div class="card" style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="status-section">
                ...
                <button class="btn btn-success upgrade-btn" onclick="openUpgradeModal()">
                    ⭐ Повысить<br><span style="font-size:11px;font-weight:bold;">4990 ₽</span>
                </button>
                <?php endif; ?>
            </div>
...
<div class="modal-overlay" id="modal-upgrade" onclick="if(event.target===this)this.classList.remove('open')">
    <div class="modal-box">
        ...
        <p style="color:#666;font-size:14px;margin:12px 0;">
            Повысьте свой статус на <b>✅ Ответственного</b> за <b>4990 ₽</b>
        </p>
...
function performUpgrade(method) {
    setUpgradeMsg('Обработка...', '#0088cc');
    const fd = new FormData();
    fd.append('action', 'upgrade');
    fd.append('method', method);
    fd.append('amount', UPGRADE_COST);

    fetch('upgrade_status_modal.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => {
            if (d.success) {
                ...
                setUpgradeMsg(d.msg || 'Ошибка', '#ef4444');
            }
        })
        .catch(() => setUpgradeMsg('Ошибка связи', '#ef4444'));
}

function showUpgradeModal(method) {
    if (method === 'qr') {
        const purpose = `Повышение статуса ID${USER_ID} ${USERNAME}`;
        const qrData = `ST00012|Name=ООО Форсаж|PersonalAcc=40702810101500033019|BankName=ООО Банк Точка|BIC=044525104|CorrespAcc=30101810745374525104|PayeeINN=7728282160|KPP=773001001|Sum=${UPGRADE_COST}00|Purpose=${purpose}`;
        
        // Используем upgrade_qr.php
        window.location.href = `upgrade_qr.php?amount=${UPGRADE_COST}&purpose=${encodeURIComponent(purpose)}&qr=${encodeURIComponent(qrData)}`;
    } else {
        // Используем upgrade_receipt.php
        window.location.href = `upgrade_receipt.php?amount=${UPGRADE_COST}&purpose=${encodeURIComponent(`Повышение статуса ID${USER_ID} ${USERNAME}`)}`;
    }
}
