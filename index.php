...
.tile { transition: transform 0.3s ease, border-color 0.3s ease, box-shadow 0.3s ease; }
.tile:hover { border-color: rgba(56,189,248,.45); transform: translateY(-6px); box-shadow: 0 16px 40px rgba(0,136,204,.25); }
        ...
        const io = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // Плитки: анимируем через Web Animations API (не CSS),
                    // чтобы hover никогда не перезапускал анимацию
                    if (entry.target.classList.contains('tile')) {
                        const idx = [...entry.target.parentElement.children].indexOf(entry.target);
                        const delay = idx * 150;
                        entry.target.style.opacity = '0';
                        setTimeout(() => {
                            const anim = entry.target.animate(
                                [
                            ...
                            anim.onfinish = () => {
                                anim.cancel();
                                // Set inline — overrides .tile { opacity:0 } permanently
                                entry.target.style.cssText += ';opacity:1!important;transform:none!important;';
                            };
                        }, delay);
                    }
                }
...
/* === ПЛИТКИ РОЛЕЙ ========================================================== */
/* Плитки: анимация управляется JS через Web Animations API — нет CSS-анимации,
   нет проблем с hover-перезапуском */
...
/* tile hover handled by base .tile transition */
